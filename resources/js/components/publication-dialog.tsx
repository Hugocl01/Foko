...
// Ajustamos la definición de Preset a id:number
interface Preset {
    id: number
    name: string
}

// Ahora preset_id es number
...
export function PostDialog({
    ...
    // Convertir valor de string a número en select
    const handleInputChange = (field: keyof PostFormData, value: any) => {
        setFormData(prev => ({
            ...prev,
            [field]: field === 'preset_id' ? Number(value) : value,
        }))
    }
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        if (toAdd.length) setFormData(prev => ({ ...prev, images: [...prev.images, ...toAdd] }))
    }
    const handleWrapperClick = () => { if (formData.images.length < maxImages) fileInputRef.current?.click() }
    ...
    const removeHashtag = (tag: string) => setFormData(prev => ({ ...prev, hashtags: prev.hashtags.filter(h => h !== tag) }))
    const handleKeyPress = (e: React.KeyboardEvent) => { if (e.key === "Enter") { e.preventDefault(); addHashtag() } }
    ...
    const FileUploadArea = () => (
        <div className={`relative border-2 border-dashed rounded-lg p-4 transition-colors cursor-pointer ${dragActive === "images" ? "border-primary bg-primary/5" : "border-muted-foreground/25 hover:border-muted-foreground/50"}`}
            ...
            onDrop={handleDrop}
        >
            <input ref={fileInputRef} type="file" accept="image/*" multiple disabled={formData.images.length >= maxImages}
                onChange={e => {
                ...
                {formData.images.length > 0 ? (
                    <div className="flex flex-col gap-2">
                        {formData.images.map((f, i) => (
                            <div key={i} className="flex items-center justify-between">
                                ...
                                <Button size="sm" variant="ghost" onClick={e => { e.stopPropagation(); removeImage(i) }}>
                                    <X className="h-4 w-4" />
                                </Button>
                            </div>
                        ))}
                        {formData.images.length < maxImages && <p className="text-xs text-muted-foreground">Puedes subir {maxImages - formData.images.length} imagen(es) más.</p>}
                    </div>
                ) : (
                    <div className="text-center">
                        ...
                        <div className="mt-2">
                            ...
                            <p className="text-xs text-muted-foreground">{userRole_id === 1 ? "Hasta 3 imágenes" : "1 imagen máxima"}</p>
                        </div>
                    </div>
                )}
            </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>{isEditing ? "Modifica tu publicación existente." : "Completa la información para publicar."}</DialogDescription>
                </DialogHeader>
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                        ...
                        <TabsContent value="info" className="space-y-4 pt-4">
                            <div className="space-y-2">
                                ...
                                <Input id="title" value={formData.title} onChange={e => handleInputChange("title", e.target.value)} placeholder="Título de la publicación" required />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="preset">Preset</Label>
                                <Select value={formData.preset_id?.toString() || ""} onValueChange={val => handleInputChange("preset_id", val)}>
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description} onChange={e => handleInputChange("description", e.target.value)} placeholder="Escribe tu descripción aquí..." rows={5} required />
                            </div>
                        </TabsContent>
                        <TabsContent value="media" className="space-y-4 pt-4">
                            <Alert><AlertDescription>Arrastra y suelta solo imágenes, o haz clic para seleccionar.</AlertDescription></Alert>
                            <FileUploadArea />
                        </TabsContent>
